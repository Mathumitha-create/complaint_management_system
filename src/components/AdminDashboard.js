...
const AdminDashboard = ({ user }) => {
      ...
      unsubscribe = onSnapshot(
        ...
        (snapshot) => {
          ...
          setStats({
            ...
            inProgress: grievanceData.filter((g) => g.status === "In Progress").length,
            resolved: grievanceData.filter((g) => g.status === "Resolved").length,
            escalated: grievanceData.filter((g) => g.status === "Escalated").length,
          });
        },
        (error) => {
          console.error("AdminDashboard Firestore onSnapshot error", { code: error.code, message: error.message });
        }
      );
    } catch (e) {
  ...
  const handleUpdateUser = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`http://localhost:5000/api/users/${editingUser.id}/role`, {
        method: "PUT",
    ...
    <div className="grievances-list">
      ...
      <div className="grievance-table">
        <div
          ...
          style={{ gridTemplateColumns: "80px 2fr 1.5fr 1fr 110px 140px 100px" }}
        >
          <div>ID</div>
          <div>{t("title")}</div>
  ...
  const renderUsersList = () => (
    <div className="grievances-list">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
        ...
        <button onClick={fetchUsers} className="action-btn refresh-btn">🔄 Refresh</button>
      </div>
      ...
      <div className="grievance-table">
        <div className="table-header" style={{ gridTemplateColumns: "1.5fr 1.5fr 1fr 1fr 100px" }}>
          <div>Name</div>
          <div>Email</div>
          <div>Role</div>
          <div>Hostel Type</div>
        ...
        {filteredUsers.map((u) => (
          <div key={u.id} className="table-row" style={{ gridTemplateColumns: "1.5fr 1.5fr 1fr 1fr 100px" }}>
            <div>{u.name}</div>
            <div>{u.email}</div>
            <div>
            ...
            <div className="action-buttons">
              <button onClick={() => handleEditUser(u)} className="icon-btn view-btn" title="Edit Role">✏️</button>
              {u.email !== 'mathupriya2006@gmail.com' && (
                <button onClick={() => handleDeleteUser(u.id)} className="icon-btn delete-btn" title="Delete User">🗑️</button>
              )}
            </div>
          </div>
        ))}
      ...
      <div className="modal-overlay" onClick={() => setShowUserModal(false)}>
        <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{ maxWidth: "500px" }}>
          ...
          <form onSubmit={handleUpdateUser} className="modal-body">
            ...
            {(editingUser.role === "warden_girls" || editingUser.role === "warden_boys") && (
              <div className="form-group" style={{ marginBottom: "1rem" }}>
                <label style={{ display: "block", marginBottom: "0.5rem" }}>Hostel Type</label>
                <select
                  value={editingUser.hostelType || ""}
                  onChange={(e) => setEditingUser({ ...editingUser, hostelType: e.target.value })}
                  className="form-input"
                  style={{ width: "100%", padding: "0.5rem" }}
                >
            ...
            <div className="modal-footer">
              <button type="submit" className="login-button">Update User</button>
      ...
      <div className="main-content">
        ...
        >
          <div className="header-title">
            <h1 style={{ color: "white", fontSize: "2rem", margin: "0 0 8px 0" }}>
              {activeTab === "dashboard" ? "Dashboard" : activeTab === "users" ? "User Management" : "All Grievances"}
            </h1>
            <p className="header-subtitle" style={{ color: "rgba(255, 255, 255, 0.9)", margin: 0 }}>
              {activeTab === "users" ? "Manage users and roles" : "Manage and monitor all grievances"}
            </p>
          </div>
